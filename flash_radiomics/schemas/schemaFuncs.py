...
from flash_radiomics import getFeatureClasses, getImageTypes
...
def _check_feature_class(value, allowed_classes=None, allow_none=False):
    if value is None:
        if allow_none:
            return True
    ...
    if not isinstance(value, dict):
        raise TypeError("Feature class selection must be a dictionary or null")
    for className, features in value.items():
        if className not in featureClasses:
            ...
            raise ValueError(msg)
        if allowed_classes is not None and className not in allowed_classes:
            msg = f"Feature Class {className} is not supported for this extraction mode"
            raise ValueError(msg)
        if features is not None:
            if not isinstance(features, list):
                msg = f"Value of feature class {className} not expected type (list)"
                raise TypeError(msg)
            get_feature_names = getattr(featureClasses[className], "getFeatureNames", None)
            if features and callable(get_feature_names):
                unrecognizedFeatures = set(features) - set(get_feature_names())
...
def checkFeatureClass(value, _rule_obj, _path):
    return _check_feature_class(value)


def checkScalarFeatureClass(value, _rule_obj, _path):
    return _check_feature_class(
        value,
        {
            "firstorder",
            "ih",
...
def checkSpatialFeatureClass(value, _rule_obj, _path):
    return _check_feature_class(
        value,
