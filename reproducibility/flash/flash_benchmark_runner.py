#!/usr/bin/env python3
"""Flash-only official benchmark runner for scalar and spatial workflows."""
    ...
    for feature_key in sorted(feature_keys):
        ...
        try:
            ...
            timings_by_feature[feature_key] = legacy._nan_timing_stats(
                note=f"flash_feature_timing_error:{type(exc).__name__}"
            )
...
def _build_segment_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Benchmark Flash segment-based extraction and write outputs into reproducibility/flash/exp."
    )
    parser.add_argument(
        "--dataset-suite",
        choices=("kits", "ibsi1", "ibsi_validation"),
        default="kits",
    )
    ...
    parser.add_argument("--segment-class-workers", type=int, default=0)
...
def _build_voxel_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Benchmark Flash spatial extraction and write outputs into reproducibility/flash/exp."
    )
    parser.add_argument(
        "--dataset-suite",
    ...
    parser.add_argument("--distances", default="1")
    parser.add_argument("--backend", choices=("cpu", "mps", "cuda"), default="cpu")
    parser.add_argument("--flash-voxel-batch", type=int, default=2048)
    parser.add_argument("--num-threads", type=int, default=0)
    parser.add_argument("--repeats", type=int, default=3)
    parser.add_argument("--repeat-workers", type=int, default=0)
    parser.add_argument("--skip-feature-timings", action="store_true")
...
def _segment_items(args) -> tuple[list[dict], list[str], list[int], dict]:
    ...
    base_settings = SEGMENT_LEGACY._base_settings(
        ...
        segment_class_workers=args.segment_class_workers,
    )
...
def _voxel_items(args) -> tuple[list[dict], list[str], list[int], dict]:
    ...
    base_settings = SPATIAL_HELPERS._base_settings(
        ...
        bin_width=args.bin_width,
        distances=distances,
        voxel_batch=args.flash_voxel_batch,
    )
...
def _segment_run(args) -> None:
    ...
    if args.dataset_suite == "ibsi1":
        exp_dir = make_experiment_dir("flash", "segment_ibsi1", args.backend)
        run_ibsi1_segment_validation(
            tool_name="flash",
            ...
            build_extractor=lambda feature_classes, settings: SEGMENT_LEGACY._build_flash_all(
                settings,
                feature_classes,
                args.backend,
                args.num_threads,
    ...
    mode_name = "segment_ibsi_validation" if args.dataset_suite == "ibsi_validation" else "segment"
    exp_dir = make_experiment_dir("flash", mode_name, args.backend)
    ...
    for item in items:
        ...
        try:
            extractor_factory = lambda: SEGMENT_LEGACY._build_flash_all(
                base_settings,
            ...
            if args.skip_feature_timings:
                feature_timings = {
                    feature_key: SEGMENT_LEGACY._nan_timing_stats("feature_timing_skipped")
                ...
                feature_timings = _measure_feature_timings(
                    ...
                    repeat_workers=args.repeat_workers,
                    spatial_mode=False,
                    build_extractor=lambda feature_class, feature_name: SEGMENT_LEGACY._build_flash_single_feature(
                        base_settings,
                        feature_class,
                        feature_name,
                        args.backend,
            ...
            overall_row["note"] = SEGMENT_LEGACY._append_note(
                overall_row["note"],
                f"flash_segment_error:{type(exc).__name__}",
            )
    ...
    overall_csv = exp_dir / "flash_segment_case_timing.csv"
    per_feature_csv = exp_dir / "flash_segment_feature_timing.csv"
    feature_values_csv = exp_dir / "flash_segment_feature_values_wide.csv"
    summary_json = exp_dir / "flash_segment_benchmark_summary.json"
...
def _voxel_run(args) -> None:
    ...
    for item in items:
        ...
        try:
            extractor_factory = lambda: SPATIAL_HELPERS._build_flash_all(
                base_settings,
                args.backend,
                args.num_threads,
            )
            ...
            if args.skip_feature_timings:
                ...
                feature_timings = _measure_feature_timings(
                    ...
                    repeat_workers=args.repeat_workers,
                    spatial_mode=True,
                    build_extractor=lambda feature_class, feature_name: SPATIAL_HELPERS._build_flash_single_feature(
                        base_settings,
                        feature_class,
                        feature_name,
                        args.backend,
            ...
            overall_row["note"] = SPATIAL_HELPERS._append_note(
                overall_row["note"],
                f"flash_voxel_error:{type(exc).__name__}",
            )
    ...
    overall_csv = exp_dir / "flash_voxel_case_timing.csv"
    per_feature_csv = exp_dir / "flash_voxel_feature_timing.csv"
    feature_values_csv = exp_dir / "flash_voxel_feature_values_wide.csv"
    map_index_csv = exp_dir / "flash_voxel_map_index.csv"
    summary_json = exp_dir / "flash_voxel_benchmark_summary.json"
